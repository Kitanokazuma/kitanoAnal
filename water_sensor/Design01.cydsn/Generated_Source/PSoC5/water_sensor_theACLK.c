/*******************************************************************************
* File Name: water_sensor_theACLK.c
* Version 2.10
*
*  Description:
...
/*******************************************************************************
* Function Name: water_sensor_theACLK_Start
********************************************************************************
*
* Summary:
...
void water_sensor_theACLK_Start(void) 
{
    ...
    water_sensor_theACLK_CLKEN |= water_sensor_theACLK_CLKEN_MASK;
	water_sensor_theACLK_CLKSTBY |= water_sensor_theACLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: water_sensor_theACLK_Stop
********************************************************************************
*
* Summary:
...
void water_sensor_theACLK_Stop(void) 
{
    ...
    water_sensor_theACLK_CLKEN &= (uint8)(~water_sensor_theACLK_CLKEN_MASK);
	water_sensor_theACLK_CLKSTBY &= (uint8)(~water_sensor_theACLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: water_sensor_theACLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((water_sensor_theACLK_CLKEN & water_sensor_theACLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = water_sensor_theACLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = water_sensor_theACLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* water_sensor_theACLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(water_sensor_theACLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(water_sensor_theACLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: water_sensor_theACLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: water_sensor_theACLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = water_sensor_theACLK_GetSourceRegister();
    uint16 oldDivider = water_sensor_theACLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = water_sensor_theACLK_CLKEN & water_sensor_theACLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(water_sensor_theACLK_DIV_PTR, clkDivider);
                water_sensor_theACLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                water_sensor_theACLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(water_sensor_theACLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((water_sensor_theACLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(water_sensor_theACLK_DIV_PTR, clkDivider);
				water_sensor_theACLK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: water_sensor_theACLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(water_sensor_theACLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: water_sensor_theACLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    water_sensor_theACLK_MOD_SRC |= modeBitMask & (uint8)water_sensor_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: water_sensor_theACLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    water_sensor_theACLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(water_sensor_theACLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: water_sensor_theACLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return water_sensor_theACLK_MOD_SRC & (uint8)(water_sensor_theACLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: water_sensor_theACLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = water_sensor_theACLK_GetDividerRegister();
    uint8 oldSrc = water_sensor_theACLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        water_sensor_theACLK_MOD_SRC |= CYCLK_SSS;
        water_sensor_theACLK_MOD_SRC =
            (water_sensor_theACLK_MOD_SRC & (uint8)(~water_sensor_theACLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: water_sensor_theACLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return water_sensor_theACLK_MOD_SRC & water_sensor_theACLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: water_sensor_theACLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    water_sensor_theACLK_PHASE = clkPhase & water_sensor_theACLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: water_sensor_theACLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return water_sensor_theACLK_PHASE & water_sensor_theACLK_PHASE_MASK;
}
